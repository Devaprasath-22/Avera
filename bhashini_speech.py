"""
Bhashini Local Speech Engine Module (ASR & TTS) for MedGemma Medical Kiosk.
Supports 10 Indian languages (EN, HI, TA, TE, BN, GU, KN, ML, MR, PA)
Optimized for offline execution on NVIDIA Jetson Orin Nano (8GB).
"""
...
# Cross-platform keyboard input import
try:
    import msvcrt
except ImportError:
    msvcrt = None
...
try:
    ...
    print(f"Missing dependency in bhashini_speech module: {exc}")
    raise exc
...
def _get_default_cache_dir() -> str:
    ...
    # Priority: 1. Workspace local models dir, 2. D:\ drive cache, 3. User home cache
    ws_models = os.path.abspath(os.path.join(os.path.dirname(__file__), "models"))
    if os.path.exists(ws_models):
        return ws_models
...
BHASHINI_MODEL_DIR = _get_default_cache_dir()
...
class BhashiniLocalASR:
    ...
    def get_model(cls, model_source: str = WHISPER_MODEL_PATH):
        ...
        print(f"Loading local Bhashini ASR model from '{resolved_source}' on {device} ({compute_type})...")
        try:
            ...
            print(f"Error loading ASR model ({resolved_source}): {exc}. Falling back to 'small' model...")
...
def listen_for_speech(language: str = "en", duration: float = 5.0) -> Optional[str]:
    lang_key = language.lower() if language.lower() in SPEECH_LANGUAGE_CODES else "en"
    whisper_lang = lang_key
    ...
    # Strategy 1: PyAudio / SpeechRecognition capture
    try:
    ...
    if audio_bytes is None:
        ...
        try:
            ...
            if max_amp <= 0.0001:
                print("[Notice] Microphone signal is zero or extremely quiet.")
                return None
    ...
    try:
        temp_path = os.path.join(tempfile.gettempdir(), "medgemma_asr_kiosk.wav")
        ...
        segments, _info = model.transcribe(temp_path, language=whisper_lang, beam_size=1, vad_filter=False)
...
class BhashiniLocalTTS:
    ...
    def get_tts_engine(cls, language: str = "en"):
        ...
        device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"Loading local Bhashini TTS engine for [{lang_key.upper()}] on {device}...")
...
def speak(text: str, language: str = "en", output_path: str = OUTPUT_WAV):
    BhashiniLocalTTS.synthesize(text, language, output_path)


# ==========================
# AUDIO PLAYBACK & STOP KEY
# ==========================
...
def play_audio(path: str = OUTPUT_WAV, stop_event=None):
